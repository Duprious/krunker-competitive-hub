...
const registerTeamSchema = z
  .object({
    ...
    discordPlayerFour: z.string().min(1, {message: "Discord username with tag is required"}).max(48, {message: "Discord name cannot exceed 48 characters"}),
    ...
    ignPlayerFour: z.string().min(1, {message: "Krunker username is required"}).max(48, {message: "Krunker username cannot exceed 48 characters"}),
...
const FourVsFourSignupForm = ({tournamentId}: {tournamentId: string}) => {
  const router = useRouter()
  const addFourVsFourTeam = trpc.teamRouter.addTeam4v4.useMutation()
  ...
  const onSubmit: SubmitHandler<RegisterTeamSchema> = async (data) => {
    ...
    const duplicatePlayer = tournamentsData?.find(tournament => tournament.id === tournamentId)?.teams.find(team => team.players.find(player => player.discordName === data.discordPlayerOne || player.discordName === data.discordPlayerTwo || player.discordName === data.discordPlayerThree || player.discordName === data.discordPlayerFour || player.discordName === data.discordSub))
    ...
    // check duplicate ign
    const duplicateIgn = tournamentsData?.find(tournament => tournament.id === tournamentId)?.teams.find(team => team.players.find(player => player.ign === data.ignPlayerOne || player.ign === data.ignPlayerTwo || player.ign === data.ignPlayerThree || player.ign === data.ignPlayerFour || player.ign === data.ignSub))
    ...
    await addFourVsFourTeam.mutateAsync({
                    ...
                    <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className='grid grid-cols-2 space-x-8'>
                          <div>
                            ...
                            <label htmlFor="playerFourDiscord" className="block mb-2 text-sm font-medium mt-4 dark:text-white text-gray-800">Discord Player 4</label>
                            <input type="text" id="playerFourDiscord" className="border sm:text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 bg-stone-100 border-gray-600 placeholder-gray-400 dark:text-white text-gray-800 focus:ring-blue-500 focus:border-blue-500" placeholder='flipbait#0001' {...register("discordPlayerFour")} />
                            {errors.discordPlayerFour && (
                              <p className="text-xs italic text-red-500 mt-2">
                                {errors.discordPlayerFour?.message}
                              </p>
                            )}
                          ...
                          <div>
                            ...
                            <label htmlFor="playerFourIGN" className="block mb-2 text-sm font-medium mt-4 dark:text-white text-gray-800">IGN Player 4</label>
                            <input type="text" id="playerFourIGN" className="border sm:text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 bg-stone-100 border-gray-600 placeholder-gray-400 dark:text-white text-gray-800 focus:ring-blue-500 focus:border-blue-500" placeholder='flipbait' {...register("ignPlayerFour")} />
                            {errors.ignPlayerFour && (
                              <p className="text-xs italic text-red-500 mt-2">
                                {errors.ignPlayerFour?.message}
                              </p>
                            )}
...
export default FourVsFourSignupForm